...
char **argv;
{
	...
	if (bufalign != 0)
		buf +=
		    (bufalign - ((long)buf % bufalign) + bufoffset) % bufalign;
	...
	if (sockbufsize) {
		if (trans) {
			if (setsockopt
			    (fd, SOL_SOCKET, SO_SNDBUF, (char *)&sockbufsize,
			     sizeof sockbufsize) < 0)
				err("setsockopt: sndbuf");
			mes("sndbuf");
		} else {
			if (setsockopt
			    (fd, SOL_SOCKET, SO_RCVBUF, (char *)&sockbufsize,
			     sizeof sockbufsize) < 0)
	...
	if (!udp) {
		...
		if (trans) {
...
#else /* BSD43 */
				if (setsockopt
				    (fd, SOL_SOCKET, options, (char *)&one,
				     sizeof(one)) < 0)
#endif
					err("setsockopt");
			}
			...
			domain = AF_INET;
			if ((fd =
			     accept(fd, (struct sockaddr *)&frominet,
				    &fromlen)) < 0)
				err("accept");
			{
				...
				fprintf(stderr, "ttcp-r: accept from %s:%d\n",
...
static void prusage(r0, r1, e, b, outp)
