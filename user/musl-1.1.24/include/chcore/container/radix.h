...
__attribute__((__unused__)) static inline int radix_del(struct radix *radix,
							u64 key,
							int delete_value)
{
	struct radix_node *node;
			       ...
			       u64 start, radix_scan_cb cb, void *data)
{
	...
	for (i = start_i; i < RADIX_NODE_SIZE; i++) {
		if (node->children[i]) {
			err = __radix_scan(node->children[i], node_level + 1,
					   start, cb, data);
			if (err)
				return err;
		}
 ...
 */
static inline int radix_scan(struct radix *radix, u64 start,
			     radix_scan_cb cb, void *cb_args)
{
