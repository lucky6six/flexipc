...
_Noreturn void __pthread_exit(void *result)
{
	...
	self->tid = 0;
	UNLOCK(self->killlock);
	__tl_unlock();	/* XXX: we won't unlock the thread list in kernel */
...
#define CHCORE_PRIO_TYPE_SHADOW ((unsigned int)(-1))	/* ChCore shadow thread prio */
#define CHCORE_MAX_PRIO		255			/* ChCore maximum priority */
#define CHCORE_DEFAULT_PRIO	CHCORE_MAX_PRIO		/* ChCore thread default priority */
 ...
 */
int __pthread_create_internal(int shadow, pthread_t *restrict res, 
			      ...
			      void *(*entry)(void *), void *restrict arg)
{
	...
	new->system_ipc_fs.conn_cap = 0;
	new->system_ipc_fs.server_id = FS_MANAGER;
...

	{
		...
		_args.pc = (shadow ? (u64)entry : (u64)(c11 ? start_c11 : start));
		_args.arg = (shadow ? (u64)arg : (u64)args);
		_args.prio = attr._a_sched? attr._a_prio: CHCORE_DEFAULT_PRIO;
		_args.tls = (u64)TP_ADJ(new);
		_args.is_shadow = shadow;
...
int __pthread_create(pthread_t *restrict res,
		     ...
		     void *restrict arg)
{
	int ret = __pthread_create_internal(0, res, attrp, entry, arg);
...
int chcore_pthread_create(pthread_t *restrict res,
			  ...
			  void *restrict arg)
{
	return __pthread_create_internal(0, res, attrp, entry, arg);
}
...
int chcore_pthread_create_shadow(pthread_t *restrict res,
				 ...
				 void *restrict arg)
{
	return __pthread_create_internal(1, res, attrp, entry, arg);
}
