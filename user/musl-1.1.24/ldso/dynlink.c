...
static void *map_library(int fd, struct dso *dso)
{
	...
	*/
	...
	/**
	 * NOTE(MK): We cannot map the file here, otherwise the file content
	...
	for (ph=ph0, i=eh->e_phnum; i; i--, ph=(void *)((char *)ph+eh->e_phentsize)) {
		...
		/**
		 * NOTE(MK): Since we are forced to use anonymous mapping,
		 * we need to copy the file content by hand.
		 */
...
void __dls3(size_t *sp)
{
	...
	if (initial_tls != builtin_tls) {
		 ...
		 */
		ipc_struct_t *system_ipc_fs = fs_ipc_struct;
		...
		memcpy(fs_ipc_struct, system_ipc_fs, sizeof(ipc_struct_t));
