...
u32 uart_recv(void)
{
	...
	while (1) {
		if (!(get32((u64) PL011_UART6_BASE + UART_FR)
		& UART_FR_RXFE))
			break;
	}
	return (get32((u64) PL011_UART6_BASE + UART_DR) & 0xFF);
}
...
void uart_send(u32 ch)
{
	/* Wait until there is space in the FIFO or device is disabled */
	while (get32((u64) PL011_UART6_BASE + UART_FR)
	       & UART_FR_TXFF) {
	}
	/* Send the character */
	put32((u64) PL011_UART6_BASE + UART_DR, (unsigned int)ch);
}

void uart_init(void)
{
	u64 base = PL011_UART6_BASE;
