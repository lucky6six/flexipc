...
static u64 get_next_tick_delta()
{
	 ...
	 * FIXME: If current time is already after the time when the first
	 * thread wakes up, the tick is not updated.
	 */
...
void handle_timer_irq(void)
{
	...
	for_each_in_list_safe(iter, tmp, sleep_node,
			      local_sleep_list) {
		/* TODO: Replace the strict comparison with error range one */
...
bool try_dequeue_sleeper(struct thread *thread)
{
	...
	lock(target_sleep_list_lock);
	/* if cb is NULL, the thread is not waiting for timeout */
